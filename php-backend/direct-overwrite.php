<?php
// Direct overwrite of api/v1/lebensmittel.php
...
function getLebensmittelList($user) {
    ...
    $result = array_map(function($item) {
        return [
            ...
            \'DIRECT_OVERWRITE_SUCCESS\' => \'MULTI_TENANT_WORKING\',
...
function createLebensmittel($user) {
    ...
    try {
        ...
        // Return success with Multi-Tenant fields
        sendSuccess([
...
if (file_put_contents($target, $correct_content)) {
    echo json_encode([
        ...
        'old_size' => strlen($backup),
        'new_size' => strlen($correct_content),
        'file_size' => filesize($target),
        'timestamp' => date('Y-m-d H:i:s')
    ]);
} else {
    echo json_encode(['ERROR' => 'Failed to overwrite file']);
}
?>
