<?php
// Final fix - directly overwrite with working version
$target = __DIR__ . '/api/v1/lebensmittel.php';

// Read current content to backup
$backup = file_get_contents($target);
file_put_contents($target . '.backup', $backup);

// Write the correct Multi-Tenant version
$correct_content = '<?php
...
function getLebensmittelList($user) {
    ...
    $result = array_map(function($item) {
        return [
            ...
            \'FINAL_FIX_SUCCESS\' => \'MULTI-TENANT-WORKING\',
...
if (file_put_contents($target, $correct_content)) {
    // Clear OpCache again
    ...
    echo json_encode([
        ...
        'new_size' => strlen($correct_content),
        'file_size' => filesize($target),
        'timestamp' => date('Y-m-d H:i:s')
    ]);
} else {
    echo json_encode(['ERROR' => 'Failed to apply final fix']);
}
?>
