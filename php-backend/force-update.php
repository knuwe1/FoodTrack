<?php
// Force update lebensmittel.php with correct content
$new_content = '<?php
...
function getLebensmittelList($user) {
    ...
    $result = array_map(function($item) {
        return [
            ...
            \'FORCE_UPDATE_SUCCESS\' => \'MULTI-TENANT-v3.0-WORKING\',
...
$target_file = __DIR__ . '/api/v1/lebensmittel.php';

if (file_put_contents($target_file, $new_content)) {
    echo json_encode([
        'SUCCESS' => 'lebensmittel.php updated successfully!',
        'file_size' => filesize($target_file),
        'timestamp' => date('Y-m-d H:i:s')
    ]);
} else {
    echo json_encode([
        'ERROR' => 'Failed to update lebensmittel.php',
        'timestamp' => date('Y-m-d H:i:s')
    ]);
}
?>
